...
func saveTableToNumpy(table TableData) {
	...
	if err := os.MkdirAll(tableDir, 0755); err != nil {
		log.Fatalf("failed to create directory for table %s: %v", table.TableName, err)
	}

	// Process each column
	for _, col := range table.Columns {
		// Create the appropriate slice based on data type
		var array interface{}
		switch col.DataType {
		case DataTypeInt:
			array = make([]int64, nrows)
		case DataTypeFloat:
			array = make([]float64, nrows)
		case DataTypeString, DataTypeDate, DataTypeTime, DataTypeUUID, DataTypeNull:
			array = make([]string, nrows)
		case DataTypeBool:
			array = make([]bool, nrows)
		default:
		...
		// Write the array to the NPY file using npyio
		switch col.DataType {
		case DataTypeInt:
			err = npyio.Write(f, array.([]int64))
		case DataTypeFloat:
			err = npyio.Write(f, array.([]float64))
		case DataTypeString, DataTypeDate, DataTypeTime, DataTypeUUID, DataTypeNull:
			err = npyio.Write(f, array.([]string))
		case DataTypeBool:
			err = npyio.Write(f, array.([]bool))
		default:
			err = npyio.Write(f, array.([]string))
		}
		...
		if err != nil {
			f.Close()
			log.Fatalf("failed to write npy file %s: %v", fileName, err)
		}

		f.Close()
		log.Printf("Column %q saved successfully to %s", col.FieldName, fileName)
	}

	log.Printf("Table %q data saved successfully to directory %s", table.TableName, tableDir)
}
